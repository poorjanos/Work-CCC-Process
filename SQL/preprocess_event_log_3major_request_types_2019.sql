...
DROP TABLE T_CCC_PA_OUTPUT_TORLES_2019;
COMMIT;

CREATE TABLE T_CCC_PA_OUTPUT_TORLES_2019
AS
   SELECT   DISTINCT
            f_paid AS case_id,
   ...
   SELECT   attrib1, attrib1eng
     FROM   (SELECT   attrib1,
               ...
               FROM   (  SELECT   attrib1, attrib1eng, COUNT (case_id) AS db
                           FROM   T_CCC_PA_OUTPUT_TORLES_2019
...
UPDATE   T_CCC_PA_OUTPUT_TORLES_2019
   SET   attrib1 = 'Egyéb'
...
DROP TABLE T_CCC_PA_OUTPUT_TAJ_2019;
COMMIT;

CREATE TABLE T_CCC_PA_OUTPUT_TAJ_2019
AS
   SELECT   DISTINCT
            f_paid AS case_id,
            f_idopont AS event_end,
...
UPDATE   T_CCC_PA_OUTPUT_TAJ_2019
   SET   attrib1 = 'Egyéb'
 WHERE   attrib1 NOT IN
...
DROP TABLE T_CCC_PA_OUTPUT_KAR_2019;
COMMIT;

CREATE TABLE T_CCC_PA_OUTPUT_KAR_2019
AS
   SELECT   DISTINCT
            f_paid AS case_id,
            f_idopont AS event_end,
...
UPDATE   T_CCC_PA_OUTPUT_KAR_2019 a
...
/* Merge top3 */
DROP TABLE T_CCC_PA_OUTPUT_TOP3_2019;
COMMIT;

CREATE TABLE T_CCC_PA_OUTPUT_TOP3_2019
AS
   SELECT   * FROM T_CCC_PA_OUTPUT_TORLES_2019
   UNION
   SELECT   * FROM T_CCC_PA_OUTPUT_TAJ_2019
   UNION
   SELECT   * FROM T_CCC_PA_OUTPUT_KAR_2019;
...
DELETE FROM   T_CCC_PA_OUTPUT_TOP3_2019
      WHERE   case_id IN
                    (SELECT   case_id
                       FROM   (  SELECT   case_id,
                               ...
                               GROUP BY   case_id)
                      WHERE   first_event_date < DATE '2019-01-01'
                              OR first_event_date >= DATE '2019-06-01');
