...
t_call_times %>%
  ...
  mutate(EVENT_MONTH = floor_date(EVENT_END, unit = "month"),
         #CASE_TYPE_EN = forcats::fct_lump(factor(CASE_TYPE_EN), n = 7),
         SEGMENT = forcats::fct_lump(paste(CASE_TYPE_EN, PRODUCT_LINE), n = 15)) %>%
  mutate(ACTIVITY_EN = case_when(ACTIVITY_EN == "Premium settlement" ~ "Provide information Premium settlement",
                                 TRUE ~ ACTIVITY_EN)) %>% 
  group_by(YEAR, EVENT_MONTH, SEGMENT) %>% 
  mutate(ACTIVITY_EN = as.character(forcats::fct_lump(ACTIVITY_EN, prop = 0.025))) %>% 
  ungroup() %>% 
  ...
  summarize(
            ...
            COST = mean(COST)) %>%
  ungroup() %>% 
  filter(SEGMENT %in% c('Premium HOUSE', 'Premium CAR')) %>% 
  select(YEAR, SEGMENT, ACTIVITY_EN, FTE) %>% 
  mutate(SEGMENT = forcats::fct_relevel(SEGMENT, 'Premium HOUSE', 'Premium CAR')) %>%
