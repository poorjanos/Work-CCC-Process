...
t_ccc_pa %>% 
  ...
  labs(
    ...
    x = "Case Type (Ordered by Case Frequency)",
    title = "Distribution of Case Type Probability per Case Type"
  )
```


# Analyze Process Structure by Case Type

Each case type is a different contact reason. Thus this part of the analysis i run for case types separately.

### Add cols to event log
```{r, cache = FALSE}
...
by_case_type <- t_ccc_pa_eventlog %>%
  ...
  mutate(
    ...
    TRACE_LENGTH_AGGREGATE = map(data, get_trace_len),
    TRACE_LENGTH_BY_CASE = map(data, get_trace_length_by_case),
    THROUGH_TIME = map(data, get_through_time)
  )
```
...
### Trace Length
```{r}
by_case_type %>% select(CASE_TYPE_EN, TRACE_LENGHT_BY_CASE) %>%
  unnest() %>%
  ggplot(
    aes(x = reorder(CASE_TYPE_EN, CASE_ID, FUN = length),
        y = trace_len)) +
...
### Throughput Time
```{r}
by_case_type %>% select(CASE_TYPE_EN, THROUGH_TIME) %>% unnest()
```
